...
return {
  ...
  dependencies = {
    { "mason-org/mason.nvim", opts = {} },
    {
      ...
      opts = {
        ensure_installed = { "pyright", "ruff", "rust_analyzer", "lua_ls" },
        automatic_enable = false,
      },
    },

    -- IMPORTANT: load this before we compute capabilities
  ...
  config = function()
    ...
    -- Ruff: let Pyright own hover/signature docs
    vim.lsp.config("ruff", {
    ...
    -- Disable pylsp by default (you already use pyright + ruff)
    ...
    -- IMPORTANT:
    -- You’re using rustaceanvim, so don’t enable rust_analyzer here.
    -- rustaceanvim will start rust-analyzer for Rust buffers.
    ...
    vim.api.nvim_create_user_command("DiagOn", function()
      set_diag(true)
      print("Diagnostics ON")
    end, {})

    vim.api.nvim_create_user_command("DiagOff", function()
      set_diag(false)
