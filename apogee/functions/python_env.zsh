...
# - Prompt env name comes from PROMPT_PY_ENV_NAME (updated via precmd hook).
...
# Hook for zsh (no PROMPT_COMMAND): keep UV_PROJECT_ENVIRONMENT updated
_apogee_install_precmd_hook() {
  autoload -Uz add-zsh-hook 2>/dev/null || true
  if (( ${+functions[add-zsh-hook]} )); then
    add-zsh-hook precmd _apogee_uv_set_envvar
  else
    # Fallback: wrap precmd if user doesn't have add-zsh-hook
    if (( ${+functions[precmd]} )); then
      local _apogee_prev_precmd='precmd'
      precmd() { _apogee_uv_set_envvar; "${_apogee_prev_precmd}"; }
    else
      precmd() { _apogee_uv_set_envvar; }
    fi
  fi
}
_apogee_install_precmd_hook
...
# zsh: update once per prompt
_apogee_install_pyname_precmd_hook() {
  autoload -Uz add-zsh-hook 2>/dev/null || true
  if (( ${+functions[add-zsh-hook]} )); then
    add-zsh-hook precmd _apogee_set_prompt_py_env_name
  else
    if (( ${+functions[precmd]} )); then
      local _apogee_prev_precmd_py='precmd'
      precmd() { _apogee_set_prompt_py_env_name; "${_apogee_prev_precmd_py}"; }
    else
      precmd() { _apogee_set_prompt_py_env_name; }
    fi
  fi
}
_apogee_install_pyname_precmd_hook
